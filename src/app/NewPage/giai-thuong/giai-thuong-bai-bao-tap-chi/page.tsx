// src/app/NewPage/giai-thuong/giai-thuong-bai-bao-tap-chi/page.tsx
'use client';
...
export default function BaiBaoKhac() {
    ...
    const fetchAwards = useCallback(async () => {
        setIsLoading(true);
        try {
            const response = await fetch('/api/giai-thuong/giai-thuong-bai-bao-tap-chi', {
                method: 'GET',
            });
            if (!response.ok) {
    ...
    const handleDelete = async (award: Award) => {
        try {
            const response = await fetch(`/api/giai-thuong/giai-thuong-bai-bao-tap-chi?id=${award.id}`, {
                method: 'DELETE',
            });
