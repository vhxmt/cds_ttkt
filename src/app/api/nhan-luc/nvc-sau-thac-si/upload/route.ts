...
const uploadDir = path.join(process.cwd(), 'public/image/nhan-luc/ncv-sau-thac-si');
...
export async function POST(req: NextRequest) {
    ...
    return new Promise<NextResponse>((resolve, reject) => {
        form.parse(incomingMessage, (err: Error, fields: Fields, files: Files) => {
            ...
            const imageUrl = `/image/nhan-luc/ncv-sau-thac-si/${path.basename(uploadedFile.filepath)}`;
